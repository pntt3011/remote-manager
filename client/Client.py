...
class Client:
    ...
    def init_components(self):
        ...
        # butConnect
        self.butConnect = tk.Button(text="Kết nối", command=self.butConnect_click)
        ...
        # butPic
        self.butPic = tk.Button(text="Chụp màn hình", command=self.butPic_click)
        self.butPic.place(height=57, width=91, x=147, y=133)

        # butKeyLock
        self.butKeyLock = tk.Button(text="Keystroke", command=self.butKeyLock_click)
    ...
    def savePicButton_click(self):
        ...
        filename = asksaveasfile(mode="wb", defaultextension=files, filetypes=files)
        if filename:
    ...
    def butProcess_click(self):
        ...
        killButton = tk.Button(
            self.process, text="Kill", command=self.killProcessButton_click
        )
        killButton.place(width=66, height=47, x=24, y=12)

        # xemButton
        xemButton = tk.Button(
            self.process, text="Xem", command=self.xemProcessButton_click
        )
        ...
        # delButton
        delButton = tk.Button(
            self.process, text="Xóa", command=self.delProcessButton_click
        )
        ...
        vsb = ttk.Scrollbar(
            self.processList, orient="vertical", command=self.processList.yview
        )
    ...
    def xemProcessButton_click(self):
        ...
        idx = 0
        for x in processls:
            self.processList.insert(parent="", index=idx, iid=idx, text="", values=x)
            idx += 1
    ...
    def butApp_click(self):
        ...
        # killButton
        killButton = tk.Button(self.app, text="Kill", command=self.killAppButton_click)
        killButton.place(width=66, height=47, x=24, y=12)

        # xemButton
        xemButton = tk.Button(self.app, text="Xem", command=self.xemAppButton_click)
        xemButton.place(width=59, height=47, x=96, y=12)

        # startButton
        startButton = tk.Button(
            self.app, text="Start", command=self.startAppButton_click
        )
        startButton.place(width=59, height=47, x=231, y=12)

        # delButton
        delButton = tk.Button(self.app, text="Xóa", command=self.delAppButton_click)
        delButton.place(width=64, height=47, x=161, y=12)
        ...
        vsb = ttk.Scrollbar(self.appList, orient="vertical", command=self.appList.yview)
        vsb.pack(side="right", fill="both")
    ...
    def xemAppButton_click(self):
        ...
        idx = 0
        for x in appls:
            self.appList.insert(parent="", index=idx, iid=idx, text="", values=x)
            idx += 1
    ...
    def butKeyLock_click(self):
        ...
        self.is_hooking = False
        # hookButton
        hookButton = tk.Button(self.keylog, text="Hook", command=self.hookButton_click)
        ...
        # delButton
        delButton = tk.Button(
            self.keylog, text="Xóa", command=self.delKeylogButton_click
        )
    ...
    def butReg_click(self):
        ...
        # butBro
        butBro = tk.Button(reg, text="Browser...", command=self.browseRegFile_click)
        ...
        def callback(*args):
            dict = {
                "Get value": self.get_value_reg_place,
    ...
    def browseRegFile_click(self):
        filename = tk.filedialog.askopenfilename(
            initialdir="/", title="Open", filetypes=(("Reg file", "*.reg"),)
        )
    ...
    def sendContext_click(self):
        if (not self.send_msg("REG")) or (
            not self.send_obj(self.txtReg.get("1.0", tk.END))
        ):
            return
